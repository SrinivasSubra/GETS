...
/* MEX function for gets_nnreg. Receives the input struct D, right hand side vector d and the regularizaion factor lambda from MATLAB. See the GETS/MATLAB/Test folder. */
...

{
    ...
    {
        mexErrMsgTxt ("Usage:  x = gets_nnreg (D, d, lambda)") ;
    }
    /*------------------------------------------------------------*/
   ...
   if ((field = mxGetField (in_struct, 0, "xx")) != NULL)
    ...
    lambda = mxGetScalar (pargin [2]) ;
    
    cs *x, *temp ;
    x = gets_nnreg (s, d, lambda) ;
