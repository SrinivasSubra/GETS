#include "gets.h"
/* gets_nnls: solve a non negative least squares problem via the Lawson Hanson algorithm.
 NNLS: min ∥d − Cv∥_2 subject to v ≥ 0
   Input treedata struct D contains the sparse representation of the data allowing for reduced storage and
   large asymptotic speed ups. d is the right hand side vector.
...
cs *gets_nnls (const treedata *D, const double *d)
{
    ...
    Csub = cs_malloc (m*smax, sizeof(double)) ; /* Csub is the VR matrix that stores householder vectors V below diagonal and upper triangular R with s columns */
    beta = cs_malloc (smax, sizeof(double)) ; /* collection of beta(i) corresponding to the householder vectors V(i) */
    b = cs_malloc (m, sizeof(double)) ;   /* right hand side that changes as householders are applied */
    ...
    for(i = 0 ; i < m ; i++)  /* need b = d on entry to gets_qrupdate for first time */
    {
        b[i] = d[i] ;
    ...
    for(i = 0 ; i < n ; i++)    /* scaling after treemult to make w = C'd */
    {
        w[i] /= diag[i] ;
    }
    ...
    while(1)
    {
        ...
        if( s+1 >= smax ) /* not enough space allocated for x, xscale, Csub, beta */
        {
            ...
            Csub = cs_realloc (Csub, m*smax, sizeof (double), &okC) ;
        ...
        gets_qrupdate (Csub, b, beta, s, A, diag, t, m) ;
        ...
        {   iter++ ;
            ...
            gets_uppersolve (Csub, m, s, xls) ; /* least squares solution written in xls */
            ...
            for(i = 0 ; i < m ; i++)   /* need b = d on entry to gets_qrfact for qr factorization from scratch */
            {
                b[i] = d[i] ;
                
            }
            
            gets_qrfact (Csub, b, beta, s, A, diag, m, x) ;
        ...
        for(i = 0 ; i < n ; i++)    /* scaling after treemult to make w = C'resid */
        {
            w[i] /= diag[i] ;
        }
